...
class NonlinearMinimization(WLSRegressor):
    ...
    def fit(self, df: pl.DataFrame, prev_const: float, prev_coef: float) -> Result:
        ...
        return self._optimize(df, initial_guess, spot, tau, future_bid, future_ask, has_futures)

    def _optimize(self, df: pl.DataFrame, initial_guess: np.ndarray, spot: float, tau: float,
        ...
        return self._convert_to_result(const, coef, spot, tau, float(r_squared), float(sse))
